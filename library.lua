-- KingRua UI Library - Mobile Fixed Version
-- Fixes: Touch/click on all components, responsive sizing, slider touch, dropdown blocking
...
function Library:NewWindow(ConfigWindow)
	...
	-- ── DropdownZone (overlay for dropdowns & close confirm) ───────────────
	...
	DropdownZone.Active                = false   -- ← lets clicks pass through when no child is hit
	...
	TabLine.BackgroundColor3  = Color3.fromRGB(0, 255, 0)
	...
	LBStroke.Color     = Color3.fromRGB(0, 255, 0)
	...
	function Tab:T(tabName)
		...
		-- Invisible full-size click region
		...
		function TabFunc:AddSection(SectionName)
			...
			SecGrad.Color = ColorSequence.new {
				...
				ColorSequenceKeypoint.new(0.52, Color3.fromRGB(0, 255, 0)),
			...
			function SectionFunc:AddToggle(cfg)
				...
				-- Full-row invisible click button
				...
				function TogFunc:Set(bool)
					self.Value = bool
					if bool then
						Library:TweenInstance(Track, 0.3, "BackgroundColor3", Color3.fromRGB(0, 255, 0))
			...
			function SectionFunc:AddSlider(cfg)
				...
				Track.Size              = UDim2.new(0, 130, 0, 10)  -- slightly taller for touch
				Instance.new("UICorner", Track).CornerRadius = UDim.new(1, 0)
				...
				ValBox.BackgroundColor3      = Color3.fromRGB(0, 255, 0)
				...
				-- Invisible large hit area on top of track (easier to touch)
				...
				-- Also handle direct track input
			...
			function SectionFunc:AddDropdown(cfg)
				...
				-- Open button (full row + selects box)
